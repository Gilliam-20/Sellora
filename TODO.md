...
| **5 — Seller product management** | Write-path migration done 2026-09-18 (`listProduct`/`updateListing`/`unlistProduct` now target `stores/{storeId}/products`, flat `listings` is dead code) — see WORKLOG.md. Still not started: variants management UI, collections, real inventory tracking, SEO fields, bulk operations, pagination, server-authorized writes |
