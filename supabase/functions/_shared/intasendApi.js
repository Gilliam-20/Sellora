...
// Set with: npx supabase secrets set INTASEND_SECRET_KEY=...
...
const INTASEND_SECRET_KEY = "INTASEND_SECRET_KEY";
...
function mpesaAmountError(amount) {
  ...
  if (value > MPESA_MAX_KES) {
    return `M-Pesa cannot take more than KES ${MPESA_MAX_KES.toLocaleString("en-US")} ` +
      "in one payment - please pay by card";
  }
  return null;
}
...
async function intasendRequest({ method = "POST", path, data }) {
  try {
    const res = await request({
      ...
      data,
      headers: {
        "Authorization": `Bearer ${requireEnv(INTASEND_SECRET_KEY)}`,
