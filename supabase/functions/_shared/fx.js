...
import { ALERTS, logAlert, logWarning } from "./logging.js";
...
// rate set/day in public.fx_rates (the same row the app reads for display)
// so checkout doesn't depend on a live external call on every request.
const FX_ROW_ID = "current";
...
/**
 * Age of a cached rate doc. The database hands `fetchedAt` back as an ISO
 * string, but `refreshFxRates` returns the doc it just wrote, where it's
 * still a Date.
...
function rateAgeMs(fetchedAt, now = Date.now()) {
  const millis = millisOf(fetchedAt);
  if (millis <= 0) return Infinity;
  return now - millis;
}
...
async function getFxDoc() {
  const row = must(await db().from("fx_rates")
      .select("base, rates, fetched_at").eq("id", FX_ROW_ID).maybeSingle());
  const cached = row ?
    { base: row.base, rates: row.rates || {}, fetchedAt: row.fetched_at } :
    null;
...
async function refreshFxRates() {
  ...
  must(await db().from("fx_rates").upsert({
    id: FX_ROW_ID,
    base: doc.base,
    rates: doc.rates,
