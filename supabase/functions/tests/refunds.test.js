...
} from "../_shared/refunds.js";
import { normalizeRefundReason } from "../_shared/intasendApi.js";
...
describe("chargedAmount", () => {
  ...
  test("a PayPal order is not refundable here - the Supabase backend has no PayPal", () => {
    assert.equal(chargedAmount(mpesaOrder({ paymentProvider: "PAYPAL" })), null);
  });
...
describe("refundedState", () => {
  ...
  test("keeps money arithmetic to the cent rather than accumulating float drift", () => {
    const order = mpesaOrder({ totalKes: 30, refundedAmount: 10.1 });
