 ...
 *
 * Everything here is pure so it can be tested without CJ or Firestore -
...
// The order document's own `status`, which the Flutter `OrderStatus` enum
...
function shouldRefreshTracking(
  ...
  const checkedAt = order.trackingCheckedAt?.toMillis?.() ?? 0;
