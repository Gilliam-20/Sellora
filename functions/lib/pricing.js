...
const margin = require("./marginPricingService");
const { REGION_CONFIG } = require("./regions");
...
// Region keys a `config/pricing.regionOverrides` entry may be filed under -
...
// The 20% target lives here, and only here - change it in Firestore
// (config/pricing) or in these defaults, never inline elsewhere.
...
/**
 * @param {*} value Raw `{ [id]: margin }` map from Firestore.
...
/**
 * @param {*} value Raw `config/pricing.regionOverrides` map from
 *   Firestore, e.g. `{ eu: { targetNetMargin: 0.25, paymentFeePercentage:
...
/**
 * Loads the central pricing config from Firestore (`config/pricing`),
...
async function getPricing() {
  ...
  const snapshot = await PRICING_DOC.get();
 ...
 * override configured in `config/pricing` affects the catalog price but not
 ...
 * module stays synchronous and never touches Firestore/network itself for
